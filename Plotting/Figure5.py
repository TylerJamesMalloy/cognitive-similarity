...
for author in emails['Author'].unique():
    for style in emails['Style'].unique():
        ...
        for cIdx, cEmail in cEmails.iterrows():
            ...
            q = pd.DataFrame([["Pruned Cosine " + cEmail['Type'], hamSim, phishSim]], columns=qColumns)
...
n_samples = 100
cosines = qdf[(qdf["Type"] == "Pruned Cosine phishing") | (qdf["Type"] == "Pruned Cosine ham")]

similarities = np.array([cosines["Ham Similarity"].to_numpy(), cosines["Phishing Similarity"].to_numpy()]).T
catrgories = cosines["Type"].to_numpy() == "Pruned Cosine phishing"
...
g.figure.suptitle("Human Participant and Pruned Cosine Similarity \n of Phishing and Ham Emails", fontsize=18)
