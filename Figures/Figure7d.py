...
groupSender = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Sender Mismatch'].apply(np.mean)
groupRequest = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Request Credentials'].apply(np.mean)
groupSubject = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Subject Suspicious'].apply(np.mean)
groupUrgent = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Urgent'].apply(np.mean)
groupOffer = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Offer'].apply(np.mean)
groupLink = emails.groupby(['Author', 'Style', 'Type'], as_index=False)['Link Mismatch'].apply(np.mean)

random_sample = decisions["UserId"].unique()[10]
...
for author in emails['Author'].unique():
    for style in emails['Style'].unique():
        hamAvgSenderMismatch = groupSender[(groupSender['Author'] == author) & (groupSender['Style'] == style) & (groupSender['Type'] == 'ham')]['Sender Mismatch'].mean()
        phishAvgSenderMismatch = groupSender[(groupSender['Author'] == author) & (groupSender['Style'] == style) & (groupSender['Type'] == 'phishing')]['Sender Mismatch'].mean() 

        hamAvgRequestCredentials = groupRequest[(groupRequest['Author'] == author) & (groupRequest['Style'] == style) & (groupRequest['Type'] == 'ham')]['Request Credentials'].mean()
        phishAvgRequestCredentials = groupRequest[(groupRequest['Author'] == author) & (groupRequest['Style'] == style) & (groupRequest['Type'] == 'phishing')]['Request Credentials'].mean() 

        hamAvgSubjectSuspicious = groupSubject[(groupSubject['Author'] == author) & (groupSubject['Style'] == style) & (groupSubject['Type'] == 'ham')]['Subject Suspicious'].mean()
        phishAvgSubjectSuspicious = groupSubject[(groupSubject['Author'] == author) & (groupSubject['Style'] == style) & (groupSubject['Type'] == 'phishing')]['Subject Suspicious'].mean() 

        hamAvgUrgent = groupUrgent[(groupUrgent['Author'] == author) & (groupUrgent['Style'] == style) & (groupUrgent['Type'] == 'ham')]['Urgent'].mean()
        phishAvgUrgent = groupUrgent[(groupUrgent['Author'] == author) & (groupUrgent['Style'] == style) & (groupUrgent['Type'] == 'phishing')]['Urgent'].mean() 

        hamAvgOffer = groupOffer[(groupOffer['Author'] == author) & (groupOffer['Style'] == style) & (groupOffer['Type'] == 'ham')]['Offer'].mean()
        phishAvgOffer = groupOffer[(groupOffer['Author'] == author) & (groupOffer['Style'] == style) & (groupOffer['Type'] == 'phishing')]['Offer'].mean() 

        hamAvgLinkMismatch = groupLink[(groupLink['Author'] == author) & (groupLink['Style'] == style) & (groupLink['Type'] == 'ham')]['Link Mismatch'].mean()
        phishAvgLinkMismatch = groupLink[(groupLink['Author'] == author) & (groupLink['Style'] == style) & (groupLink['Type'] == 'phishing')]['Link Mismatch'].mean() 

        hamAvg = [ hamAvgSenderMismatch, hamAvgRequestCredentials, hamAvgSubjectSuspicious, hamAvgUrgent, hamAvgOffer, hamAvgLinkMismatch]
        phishAvg = [phishAvgSenderMismatch, phishAvgRequestCredentials, phishAvgSubjectSuspicious, phishAvgUrgent, phishAvgOffer, phishAvgLinkMismatch]
        ...
        for cIdx, cEmail in cEmails.iterrows():
            ...
            semantic = [cEmail['Sender Mismatch'], cEmail['Request Credentials'], cEmail['Subject Suspicious'], cEmail['Urgent'], cEmail['Offer'], cEmail['Link Mismatch']]

            hamSim   = sim(semantic, hamAvg)
            phishSim = sim(semantic, phishAvg)

            hamSim = (hamSim + (2*human_ham_similarity)) / 3
            phishSim = (phishSim + (2*human_phish_similarity)) / 3

            q = pd.DataFrame([["Individual Human " +  cEmail['Type'], human_ham_similarity, human_phish_similarity]], columns=qColumns)
        ...
        for cIdx, cEmail in cEmails.iterrows():
            ...
            q = pd.DataFrame([["Semantic Cosine " + cEmail['Type'], hamSim, phishSim]], columns=qColumns)
            qdf = pd.concat([qdf, q], ignore_index=True)
...
n_samples = 100
cosines = qdf[(qdf["Type"] == "Semantic Cosine phishing") | (qdf["Type"] == "Semantic Cosine ham")]

similarities = np.array([cosines["Ham Similarity"].to_numpy(), cosines["Phishing Similarity"].to_numpy()]).T
catrgories = cosines["Type"].to_numpy() == "Semantic Cosine phishing"
...
g = sns.jointplot(data=qdf, x="Ham Similarity", y="Phishing Similarity", hue_order=["Semantic Cosine phishing", "Individual Human phishing", "Semantic Cosine ham", "Individual Human ham"], palette=combined_palette, hue="Type", xlim = (-0.1,1.1), ylim = (-0.1,1.1)).plot_joint(sns.kdeplot, zorder=5, n_levels=5)
g.figure.suptitle("Individual Participant and Semantic Similarity \n of Phishing and Ham Emails", fontsize=18)
