...
for author in emails['Author'].unique():
    for style in emails['Style'].unique():
        ...
        qColumns = ["EmailId", "Type", "Ham Similarity", "Phishing Similarity"]
        ...
        for cIdx, cEmail in cEmails.iterrows():
            ...
            q = pd.DataFrame([[cEmail["EmailId"], "Weighted Cosine " + cEmail['Type'], hamSim, phishSim]], columns=qColumns)
...
for idx, decision in mdf.iterrows():
    ...
    hamSim = qdf[qdf["EmailId"] == decision["EmailId"]]["Ham Similarity"].iloc[0]
    phishSim = qdf[qdf["EmailId"] == decision["EmailId"]]["Phishing Similarity"].iloc[0]

    human_phish_similarity = (human_phish_similarity + 2*phishSim) / 3
    human_ham_similarity = (human_ham_similarity + 2*hamSim) / 3
    
    q = pd.DataFrame([[decision["EmailId"], "Individual " +  decision['EmailType'], human_ham_similarity, human_phish_similarity]], columns=qColumns)
    qdf = pd.concat([qdf, q], ignore_index=True)
...
p.fig.suptitle("Inidividaul and Instance-Based Cosine Similarity \n of Phishing and Ham Emails")
p.ax_joint.collections[0].set_alpha(0.5)
p.fig.tight_layout()
p.fig.subplots_adjust(top=0.95) # Reduce plot to make room
